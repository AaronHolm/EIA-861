...
def getCapacity(url):
	...
	sheet = pd.read_excel(io.BytesIO(data), sheetname='State Totals-All Months', skiprows=3, parse_cols="A:H")
...
def getEnergySoldBack(url):
	data = requests.get(url).content
	sheet = pd.read_excel(io.BytesIO(data), sheetname='State Totals-All Months', skiprows=3, parse_cols="A:C,O:R")
	sheet.rename(columns={'Year':'year','Month':'month','State':'state'}, inplace=True)
	#sheet.columns = ['year', 'month', 'state', 'Residential', 'Commercial', 'Industrial', 'Transportation']
	f2013esb = pd.melt(sheet, id_vars=['year', 'month', 'state'], value_vars=['Residential', 'Commercial', 'Industrial', 'Transportationi'], var_name='sector', value_name='value')
	f2013esb['units'] = 'MWh'
	f2013esb['nem'] = 'Yes'
	f2013esb['type'] = 'PV'
...
def getCount(url):
	data = requests.get(url).content
	sheet = pd.read_excel(io.BytesIO(data), sheetname='State Totals-All Months', skiprows=3, parse_cols="A:C,J:M")
	sheet.rename(columns={'Year':'year','Month':'month','State':'state'}, inplace=True)
	#sheet.columns = ['year', 'month', 'state', 'Residential', 'Commercial', 'Industrial', 'Transportation']
