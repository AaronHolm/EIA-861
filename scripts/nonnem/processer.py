...
def processer(df_in):
		...
		for sheet in t1['sheet'].unique():
			...
			for sector in t2['sector'].unique():
				...
				for year in sectors['year'].unique():
					...
					if len(y[y['incremental_positives'] != 0]['incremental_positives']) == 0:
						y.loc[:, 'yearly_avg'] = 0
					else:
	...
	df = df[['state', 'sheet', 'nem', 'units', 'year', 'month', 'sector', 'cumulative_value', 'shifted_value', 'incremental_value', 'incremental_positives', 'incremental_negatives', 'yearly_avg', 'result_value']]
	return df
